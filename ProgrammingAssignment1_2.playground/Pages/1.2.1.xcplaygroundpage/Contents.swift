...
let fullNameArr:[String] = studentsStr.components(separatedBy: "; ")
var components:[String] = []
for student in fullNameArr {
...
var list_with_rating:[Int] = []
for (group, students) in studentsGroups {
    studentPoints[group] = [:];
    for student in students{
        list_with_rating = []
        for max_rating in points {
            list_with_rating.append(randomValue(maxValue: max_rating))
        }
        studentPoints[group]![student] = list_with_rating;
    }
}
...
for (group, students) in studentPoints {
    ...
    for student in students{
        ...
        sumPoints[group]![student.key] = sum_rating;
    }
}
...
for (group, students) in sumPoints {
    sum_rating = 0;
    number_of_students_in_group = 0;
    for student in students{
        sum_rating += student.value;
        number_of_students_in_group += 1;
    }
    groupAvg[group] = Float(sum_rating)/Float(number_of_students_in_group); // I hope there will be a least one student in every group.
}
